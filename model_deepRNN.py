...
class Attention(object):
    ...
    def init_params(self, options):
        ...
        if options['init_tulstm']:        
            params = self.layers.get_layer('ff')[0](
                 params, prefix='ff_state', nin=ctx_dim, nout=options['tu_dim'])
            params = self.layers.get_layer('ff')[0](
                 params, prefix='ff_memory', nin=ctx_dim, nout=options['tu_dim'])

        params = self.layers.get_layer('lstm')[0](params, nin=options['dim_word'],
                                                  dim=options['tu_dim'], prefix='tu_lstm')
        params = self.layers.get_layer('lstm_cond_softatt')[0](options, params, nin=options['tu_dim'],
                                                       dim=options['mu_dim'], dimctx=ctx_dim,
                                                       prefix='mu_lstm')
        # readout
        params = self.layers.get_layer('ff')[0](
            params, prefix='ff_logit_lstm',
            nin=options['mu_dim'], nout=options['dim_word'])
        if options['ctx2out']:
            params = self.layers.get_layer('ff')[0](
                params, prefix='ff_logit_ctx',
                nin=ctx_dim, nout=options['dim_word'])
        if options['n_layers_out'] > 1:
            for lidx in xrange(1, options['n_layers_out']):
                params = self.layers.get_layer('ff')[0](
                    params, prefix='ff_logit_h%d'%lidx,
                    nin=options['dim_word'], nout=options['dim_word'])
        params = self.layers.get_layer('ff')[0](
            params, prefix='ff_logit',
    ...
    def build_model(self, tparams, options):
        ...
        tu_init_memory = None
        if options['init_tulstm']:
            tu_init_state = self.layers.get_layer('ff')[1](
                tparams, ctx_mean, prefix='ff_state', activ='tanh')
            tu_init_memory = self.layers.get_layer('ff')[1](
                tparams, ctx_mean, prefix='ff_memory', activ='tanh')
        
        # decoder
        tu_lstm = self.layers.get_layer('lstm')[1](tparams, emb,
                                                   init_state=tu_init_state, 
                                                   init_memory = tu_init_memory,        
                                                   mask=mask, prefix='tu_lstm')
                                                   
        mu_lstm = self.layers.get_layer(options['att_type'])[1](options, tparams, tu_lstm[0],
                                                        mask=mask, context=ctx_,ctx_mean=ctx_mean,
                                                        one_step=False,
                                                        trng=trng,
                                                        use_noise=use_noise,
                                                        prefix='mu_lstm')
    ...
    def train(self,
        ...
        trng, use_noise, x, mask, ctx, mask_ctx, alphas,\
        cost, extra = \
        ...
        for eidx in xrange(max_epochs):
            ...
            for idx in self.engine.kf_train:
                ...
                if validFreq != -1 and np.mod(uidx, validFreq) == 0:
                    ...
                    #np.savetxt(save_model_dir+'alphas_list.txt',alphas_list,fmt=['%s']*alphas_list.shape[1],newline='\n')
                    train_err = -1
                    train_perp = -1
                    valid_err = -1
                    valid_perp = -1
